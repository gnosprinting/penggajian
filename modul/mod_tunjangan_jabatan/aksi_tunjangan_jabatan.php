...
if (empty($_SESSION['id_pengguna']) AND empty($_SESSION['passuser'])) {
    ...
    if ($module == 'tunjangan_jabatan' AND $act == 'tambah') {
        ...
        $cek = mysqli_query($koneksi,"SELECT * FROM tunjangan_jabatan WHERE id_jenis_tunjangan = '$jenis_tunjangan' AND nip = '$_POST[nip]'");
        if (mysqli_num_rows($cek) > 0) {
            ...
            $query = mysqli_query($koneksi,"INSERT INTO tunjangan_jabatan(id_jenis_tunjangan, nip, besar_tunjangan)
                            VALUES('$jenis_tunjangan', '$_POST[nip]' , '$besar_tunjangan')")or die(mysqli_error());
            if ($query) {
        ...
        $query = mysqli_query($koneksi,"UPDATE tunjangan_jabatan SET jenis_tunjangan_jabatan = '$_POST[jenis_tunjangan_jabatan]'
        ...
        $query = mysqli_query($koneksi,"DELETE FROM tunjangan_jabatan WHERE id_tunjangan_jabatan='$_GET[id]'");

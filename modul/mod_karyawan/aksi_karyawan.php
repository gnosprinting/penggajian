...
if (empty($_SESSION['id_pengguna']) AND empty($_SESSION['passuser'])) {
    ...
    if ($module == 'karyawan' AND $act == 'tambah') {
        ...
        $cek = mysqli_query($koneksi,"SELECT nip FROM karyawan WHERE nip = '$nip'");
            ...
            //JIKA FOTO ADA N FORMAT TIDAK SALAH
                ...
                $query = mysqli_query($koneksi,"INSERT INTO karyawan(
                            ...
                            VALUES(
                                    ...
                                    '$password',
                                    '$file',
                                    '$_POST[id_jabatan]')") or die(mysqli_error());
                if ($query) {
                    header('location:../../index.php?modul=' . $module . '&pesan=tambah');
                } else {
                    header('location:../../index.php?modul=' . $module . '&pesan=error');
    ...
    elseif ($module == 'karyawan' AND $act == 'ubah') {
        ...
        if($_POST['konf_password'] == ''){
            $niph = $_POST['niph'];
            mysqli_query($koneksi,"UPDATE karyawan SET nama_karyawan         = '$_POST[nama_karyawan]',
                                         jk             = '$_POST[jk]',
                                    ...
                                    WHERE nip      = '$niph'")or die(mysqli_error());

            mysqli_query($koneksi,"UPDATE karyawan SET nip               = '$_POST[nip]'
                                    WHERE nama_karyawan      = '$_POST[nama_karyawan]'
                                    AND tgl_lahir = '$tgl_lahir'")or die(mysqli_error());
        }else{
            ...
            mysqli_query($koneksi,"UPDATE karyawan SET
                                        nip      = '$_POST[nip]',
                                         ...
                                         id_jabatan      = '$_POST[id_jabatan]',
                                         password      = '$password'
                                    WHERE nip      = '$_POST[niph]'")or die(mysqli_error($koneksi));

            // mysqli_query($koneksi,"UPDATE karyawan SET nip               = '$_POST[nip]'
            //                         WHERE nama_karyawan      = '$_POST[nama_karyawan]'
            //                         AND tgl_lahir = '$tgl_lahir'")or die(mysqli_error($koneksi));
            }
        }
            header('location:../../index.php?modul=' . $module . '&pesan=ubah');
    } elseif ($module == 'karyawan' AND $act == 'hapus') {
        $data = mysqli_fetch_array(mysqli_query($koneksi,"SELECT foto_karyawan FROM karyawan WHERE nip='$_GET[id]'"));
        if ($data['foto_karyawan'] != 'file/default_user.png') {
            $_SESSION['foto'] = substr($data['foto_karyawan'], 15);

            //HAPUS DATA GURU
            $query = mysqli_query($koneksi,"DELETE FROM karyawan WHERE nip='$_GET[id]'");
            if ($query) {
                unlink("../../file/foto_karyawan/$_SESSION[foto]");
                //HAPUS DATA PENGGUNA
                mysqli_query($koneksi,"DELETE FROM pengguna WHERE username='$_GET[id]'");

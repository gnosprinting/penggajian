...
if (empty($_SESSION['id_pengguna']) AND empty($_SESSION['passuser'])) {
    ...
    if ($module == 'absensi' AND $act == 'tambah') {
        ...
        $cek = mysqli_query($koneksi,"SELECT * FROM absensi WHERE tgl_absensi = '$tgl_absensi'") or die(mysqli_error());
    ...
    elseif ($module == 'absensi' AND $act == 'ubah') {
        mysqli_query($koneksi,"UPDATE absensi SET kehadiran               = '$_POST[kehadiran]',
        ...
        header('location:../../index.php?modul=detail_absensi&&id='.$_POST[nip].'&&pesan=ubah');
            ...
            $query = mysqli_query($koneksi,"DELETE FROM absensi WHERE id_absensi='$_GET[id]'");
            if ($query) {

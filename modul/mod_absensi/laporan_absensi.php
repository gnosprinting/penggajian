...
$tahun = $_POST['tahun'];
//QUERY ULANGAN
$query_absensi = mysqli_query($koneksi,"SELECT substr(tgl_absensi,9,2) AS tgl FROM absensi
...
//Data
    ...
    while ($data = mysqli_fetch_array($query_karyawan)) {
        ...
        $query_jlh_kehadiran = mysqli_query($koneksi,"SELECT * FROM absensi
									  WHERE nip = '$data[nip]' AND kehadiran = 'Hadir' AND MONTH(tgl_absensi) = '$bulan' AND YEAR(tgl_absensi) = '$tahun'") or die(mysqli_error());
    ...
    foreach ($data_result as $row) {
        ...
        $query_tgl = mysqli_query($koneksi,"SELECT tgl_absensi FROM absensi
							WHERE substr(tgl_absensi,1,4) = '$tahun'
							AND substr(tgl_absensi,6,2) = '$bulan'
							GROUP BY tgl_absensi") or die(mysqli_error());
        $no_tgl = 0;
        while ($data_tgl = mysqli_fetch_array($query_tgl)) {
            $hadir_nip = mysqli_query($koneksi,"SELECT * FROM absensi WHERE nip = $row[nip] AND tgl_absensi = '$data_tgl[tgl_absensi]'") or die(mysqli_error());

...
if (empty($_SESSION['id_pengguna']) AND empty($_SESSION['passuser'])) {
    ...
    if ($module == 'jabatan' AND $act == 'tambah') {
        ...
        $cek = mysqli_query($koneksi,"SELECT nama_jabatan FROM jabatan WHERE nama_jabatan = '$jabatan'");
        if (mysqli_num_rows($cek) > 0) {
            ...
            $query = mysqli_query($koneksi,"INSERT INTO jabatan(nama_jabatan, gapok)
        ...
        $query = mysqli_query($koneksi,"UPDATE jabatan SET nama_jabatan = '$_POST[nama_jabatan]', gapok = '$_POST[gapok]'
